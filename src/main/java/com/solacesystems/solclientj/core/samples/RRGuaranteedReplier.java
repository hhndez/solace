...
public class RRGuaranteedReplier extends AbstractSample {
	...
	protected void run(String[] args, SessionConfiguration config, Level logLevel)
		...
		if (queueName != null) {
			...
			print("Will provision a topic [" + topicName
					+ "] on topicEndpoint [" + SampleUtils.SAMPLE_TOPICENDPOINT
					+ "]");
			endpoint = Solclient.Allocator
					.newTopicEndpoint(SampleUtils.SAMPLE_TOPICENDPOINT,endpointProperties);
		...
		//Si no estan creadas en el servidor estas lineas nos ayudaran a crear dinamicamente
		//
		/*
	...
	protected void finish(int status) {
		...
		//Mismo caso , si fue creada dinamicamente, se habilitaran estas lineas
		/*
