...
public class RRDirectRequester extends AbstractSample {
	...
	public void sendRequests(int maxRequestMessages, String aDestinationName) {
		...
		Topic topicReply = Solclient.Allocator.newTopic("abcd");
		...
		//If the set the reply to, we also need to subscribe to.
		txMessageHandle.setReplyTo(topicReply);
		sessionHandle.subscribe(topicReply, SolEnum.SubscribeFlags.WAIT_FOR_CONFIRM, 0);
